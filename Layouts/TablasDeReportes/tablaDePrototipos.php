<div id="tablaDePrototipos"  class="scroll table-responsive border border-success border-opacity-10 rounded pt-2 px-2 pb-5 mt-4">
<table class="table">
    <tr class="head-table">
    ...
    <tr>
        <?php
            require "../../modelo/prototiposHistorico.php";

            while ($fetchPrototiposHistorico=mysqli_fetch_assoc($resPrototiposHistorico)) {
                $idPonencia=$fetchPrototiposHistorico['id_ponencia'];
                ...
                $tituloPonencia=$fetchPrototiposHistorico['titulo_ponencia'];
                $tipoPonencia=$fetchPrototiposHistorico['categoria_ponencia'];
                $idCongreso=$fetchPrototiposHistorico['id_congreso'];
                $categoriaPonencia=$fetchPrototiposHistorico['categoria'];
                $fechaRegistroPonencia=$fetchPrototiposHistorico['fecha_registro_ponencia'];
                $videoPonencia=$fetchPrototiposHistorico['video_ponencia'];

                //Da formato de fecha
                $date = date_create($fechaRegistroPonencia);
                $fechaRegistroPonenciaFormato=date_format($date,"Y/m/d H:i");

                $idUsuarioRegistra=$fetchPrototiposHistorico['id_usuario_registra'];
                $idUsuarioEvalua=$fetchPrototiposHistorico['id_usuario_evalua'];
